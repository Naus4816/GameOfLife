...
def fitRatio(parent_shape: tuple[int, ...], fit_shape: tuple[int, ...]) -> float:
    ...
    """
    # print(parent_shape, fit_shape, min(p / f for p, f in zip(parent_shape, fit_shape))) # TODO remove
...
def centerCoord(coord: tuple[int, ...], allocated_shape: tuple[int, ...], fit_shape: tuple[int, ...]) -> tuple[int, ...]:
    ...
    """
    # print(coord, allocated_shape, fit_shape, (allocated_shape[0] - fit_shape[0]) // 2, (allocated_shape[1] - fit_shape[1]) // 2) # TODO remove
